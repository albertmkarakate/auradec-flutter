...
/**
 * Embedded MediaStore plugin — Namida namida_storage_android pattern.
 * Queries Android MediaStore for all audio files.
 * Returns folder list and track list via MethodChannel.
 */
class MediaStorePlugin : FlutterPlugin, MethodChannel.MethodCallHandler {
    ...
    override fun onAttachedToEngine(flutterPluginBinding: FlutterPlugin.FlutterPluginBinding) {
        ...
        channel = MethodChannel(flutterPluginBinding.binaryMessenger, "app.auradec/media_store")
    ...
    override fun onMethodCall(call: MethodCall, result: MethodChannel.Result) {
        when (call.method) {
            ...
            "getArtwork"    -> getArtwork(call.argument("uri") ?: "", result)
            else            -> result.notImplemented()
        }
    }

    private fun getArtwork(uri: String, result: MethodChannel.Result) {
        if (uri.isEmpty()) { result.success(null); return }
        try {
    ...
    private fun scanTracks(allowedFolders: List<String>?, result: MethodChannel.Result) {
        ...
        try {
            val cursor: Cursor? = ctx.contentResolver.query(
                ...
                "${MediaStore.Audio.Media.TITLE} ASC COLLATE NOCASE"
            )
            cursor?.use { c ->
                ...
                while (c.moveToNext()) {
                    ...
                    t["filePath"]    = data
                    t["artUri"]      = artUri
                    t["title"]       = sanitize(c.getString(idxTitle))  ?: titleFromPath(data)
                    t["artist"]      = sanitizeArtist(c.getString(idxArtist))
                    t["albumArtist"] = if (idxAlbumArt >= 0) sanitize(c.getString(idxAlbumArt)) ?: "" else ""
                    t["album"]       = sanitize(c.getString(idxAlbum))  ?: "Unknown Album"
                    t["durationMs"]  = durMs
                    t["bitrate"]     = if (bitrate > 0) bitrate else if (durMs > 0) ((size * 8L) / (durMs / 1000L * 1000L)).toInt() else 0
                    t["sampleRate"]  = 44100
                    t["fileSize"]    = size
                    t["year"]        = c.getInt(idxYear)
                    t["trackNo"]     = if (idxTrack >= 0) c.getInt(idxTrack) % 1000 else 0
    ...
    private fun getFolders(result: MethodChannel.Result) {
        ...
        val folderMap = linkedMapOf<String, LongArray>() // path → [count, albumId]

        try {
            val cursor = ctx.contentResolver.query(collection, projection, selection, null,
            ...
            cursor?.use { c ->
                ...
                while (c.moveToNext()) {
                    ...
                    entry[0]++
                    if (entry[1] == 0L) entry[1] = albumId
                }
            }

            val artBase = "content://media/external/audio/albumart"
            val folders = folderMap.map { (path, v) ->
                mapOf(
                    "path"   to path,
                    "name"   to if (path.contains("/")) path.substringAfterLast("/") else path,
                    "count"  to v[0].toInt(),
    ...
    private fun sanitize(s: String?): String? {
        if (s == null || s.isBlank() || s == "<unknown>") return null
        return s.trim()
    }

    private fun sanitizeArtist(s: String?): String {
        return sanitize(s) ?: "Unknown Artist"
    }
